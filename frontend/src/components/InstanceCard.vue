  ...
  <article class="instance-card">
    ...
    <div class="flex items-start justify-between gap-4">
      <div class="min-w-0 flex-1">
        ...
        <div class="mt-1 truncate font-mono text-xs text-slate-400">{{ instance.instance_id }}</div>
      </div>
    ...
    <div class="traffic-panel mt-5">
      <div class="flex items-center justify-between gap-3 text-xs"><span class="font-semibold text-slate-500">本月流量</span><span class="font-mono font-bold" :class="trafficColor">{{ instance.traffic_used_gb?.toFixed(2) || '0.00' }} <span class="font-normal text-slate-400">/ {{ account?.traffic_limit_gb || 200 }} GB</span></span></div>
    ...
    <div class="billing-panel mt-4"><div class="flex items-center justify-between"><span class="panel-label">账单摘要</span><span v-if="billingLoading" class="h-3.5 w-3.5 animate-spin rounded-full border-2 border-accent/30 border-t-accent"></span></div><div v-if="billingError" class="mt-2 text-xs text-danger">{{ billingError }}</div><div v-else-if="!billingLoading" class="mt-3 grid grid-cols-2 gap-3"><div><span class="detail-label">账户余额</span><strong class="font-mono" :class="(billing?.balance?.available_amount ?? 0) < 1 ? 'text-danger' : 'text-success'">{{ billing?.balance?.symbol }}{{ billing?.balance?.available_amount ?? '—' }}</strong></div><div><span class="detail-label">本月待还</span><strong class="font-mono" :class="(billing?.bill?.total_outstanding ?? 0) > 0 ? 'text-warning-dark' : 'text-slate-700'">{{ billing?.bill?.symbol }}{{ billing?.bill?.total_outstanding ?? '—' }}</strong></div></div></div>
...
async function loadBilling() { if (!props.account) return; billingLoading.value = true; billingError.value = ''; try { billing.value = await store.getBilling(props.account.id) } catch (e) { billingError.value = '账单获取失败，请检查账户权限' } finally { billingLoading.value = false } }
...
.region-label { max-width: 135px; overflow: hidden; color: #64748b; text-overflow: ellipsis; white-space: nowrap; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: 10px; }
