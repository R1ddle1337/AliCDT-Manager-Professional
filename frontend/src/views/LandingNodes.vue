...
async function remove(node) { if (!confirm(`确认删除落地节点“${node.name}”？\n如果它正在被服务或入口池使用，相关路由会自动停用。`)) return; try { await store.deleteLandingNode(node.id); delete relayLinks[node.id]; showMessage('节点已删除，相关路由已刷新') } catch (e) { showMessage(e.response?.data?.error || '删除节点失败，请稍后重试', 'error') } }
