<template>
  <div class="landing-page space-y-5 fade-in">
    ...
    <div v-if="!store.landingNodes.length" class="card empty-panel">
      <h2 class="font-semibold text-slate-800">还没有落地节点</h2>
...
async function remove(node) { if (!confirm(`确认删除落地节点“${node.name}”？\n如果它是入口池的最后一个落地目标，入口池和关联 DNS 也会一并删除。`)) return; try { await store.deleteLandingNode(node.id); delete relayLinks[node.id]; showMessage('节点已删除，相关路由和入口池已刷新') } catch (e) { showMessage(e.response?.data?.error || '删除节点失败，请稍后重试', 'error') } }
