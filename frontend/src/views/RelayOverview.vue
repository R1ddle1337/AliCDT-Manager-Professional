<template>
  <div class="space-y-5 fade-in">
    ...
    <section class="grid grid-cols-2 gap-3 xl:grid-cols-4">
      ...
      <div class="metric-card"><span>转发服务</span><strong>{{ store.services.length }}</strong><small>{{ enabledServices }} 已启用</small></div>
    ...
    <section class="overview-grid">
      <div class="card overflow-hidden">
        ...
        <div v-if="store.services.length === 0" class="empty-panel">还没有转发服务</div>
        <div v-else class="divide-y divide-slate-100">
          <div v-for="service in store.services" :key="service.id" class="service-row">
            <div class="min-w-0"><div class="truncate font-semibold text-slate-800">{{ service.name }}</div><div class="mt-1 truncate text-xs text-slate-400">{{ entryAddress(service) }}</div></div>
            <span class="protocol-tag">{{ service.network.toUpperCase() }}</span><span class="mode-tag">{{ modeLabel(service.mode) }}</span>
            <div class="text-right"><div class="text-xs font-semibold" :class="service.enabled ? 'text-success' : 'text-slate-400'">{{ service.enabled ? '运行中' : '已停用' }}</div><div class="mt-1 text-[10px] text-slate-400">{{ service.targets.length }} 个目标</div></div>
          </div>
        </div>
      </div>
      ...
      <aside class="space-y-4">
        ...
        <div class="card p-5"><div class="panel-title"><div><h2>协议透传</h2><p>端到端加密不在中转层解密</p></div><span class="panel-code">L4</span></div><div class="mt-4 flex flex-wrap gap-2"><span v-for="item in protocols" :key="item" class="protocol-chip">{{ item }}</span></div></div>
      </aside>
    </section>
  </div>
</template>
...
const enabledServices = computed(() => store.services.filter(service => service.enabled).length)
const targetCount = computed(() => store.services.reduce((sum, service) => sum + service.targets.length, 0))
