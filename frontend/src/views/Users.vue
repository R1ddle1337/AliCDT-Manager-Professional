  ...
  <div class="users-page fade-in">
    ...
    <Modal v-if="showGroupForm" size="large" @close="showGroupForm = false">
      <form class="space-y-5" @submit.prevent="saveGroup">
        <div><h2 class="mt-1 text-lg font-bold text-slate-900">为 {{ groupUser?.display_name }} 分配入口端口组</h2></div>
        <div class="form-grid">
    ...
    <Modal v-if="showQuotaForm" size="large" @close="showQuotaForm = false">
      <div class="space-y-5">
        <div><h2 class="mt-1 text-lg font-bold text-slate-900">{{ quotaUser?.display_name }} · 额度与用量流水</h2></div>
