...
- Agent 用户流量控制：独立转发服务可绑定用户，按仅上传、仅下载或双向合计进行字节级计量，额度耗尽后在 Relay 本地停止传输
...
为用户绑定一个独立转发服务后，用量来源会切换为 Agent 实测：上传是“用户 → 落地”，下载是“落地 → 用户”，双向会将两者相加。计量周期按北京时间自然月自动重置，也可由管理员手动清零。该计数是代理实际转发的应用数据字节，不包含 IP/TCP 重传等计费层开销，因此可用于本地硬额度控制，但与阿里云最终 CDT 账单可能存在少量差异。为避免多 Relay 并发重复使用同一额度，精确用户额度当前只支持一个用户绑定一个独立转发服务，入口池继续使用账户级 CDT 保护。
